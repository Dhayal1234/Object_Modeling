...
public class LeaderBoardCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
        if (tokens.size() != 2) {
            throw new IllegalArgumentException("Invalid number of tokens for leaderboard command");
        }

        String scoreOrderStr = tokens.get(1);
        ScoreOrder scoreOrder = ScoreOrder.valueOf(scoreOrderStr.toUpperCase());
